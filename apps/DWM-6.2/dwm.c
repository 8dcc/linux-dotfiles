...
enum { SchemeNorm, SchemeSelTag, SchemeSelTitle, SchemeStatus }; /* color schemes */
...
drawbar(Monitor *m)
{
	...
	for (i = 0; i < LENGTH(tags); i++) {
		w = TEXTW(tags[i]);
		drw_setscheme(drw, scheme[MWS(m).tagset[MWS(m).seltags] & 1 << i ? SchemeSelTag : SchemeNorm]);
...
focus(Client *c)
{
	...
	if (c) {
		...
		attachstack(c);
		grabbuttons(c, 1);
		XSetWindowBorder(dpy, c->win, scheme[SchemeSelTitle][ColBorder].pixel);
		setfocus(c);
	} else {
